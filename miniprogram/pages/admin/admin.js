  ...
  executeDraw() {
    ...
    wx.showModal({
      ...
      success: async (res) => {
        ...
        try {
          const drawRes = await wx.cloud.callFunction({ name: 'adminManager', data: { action: 'executeDraw', drawCount: count, poolType } });
          if (drawRes.result.success) {
            wx.showModal({ title: `🎯 ${poolType}池开奖成功！`, content: `本次抽中的是：\n${drawRes.result.winnerNames}`, showCancel: false });
            this.fetchDashboardData();
          } else {
